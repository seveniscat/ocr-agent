...
# 标记为"伪目标"(不对应真实文件)
.PHONY: help install venv up dev-up down restart status health log \
...
test: ## 跑一遍 OCR 验证。用法: make test URL=https://... 或 make test FILE=本地图.png
	@if [ -z "$(URL)" ] && [ -z "$(FILE)" ]; then \
		echo "✗ 请提供图片:make test URL=https://... 或 make test FILE=本地图.png" >&2; exit 1; \
	fi
	@if [ -n "$(FILE)" ]; then \
		echo "→ 上传文件:$(FILE)" >&2; \
		curl -s --max-time 120 -F "file=@$(FILE)" http://127.0.0.1:$(PORT)/analyze; \
	else \
		echo "→ 下载 URL:$(URL)" >&2; \
		curl -s --max-time 120 -F "url=$(URL)" http://127.0.0.1:$(PORT)/analyze; \
	fi | $(PP)
...
annotate: ## 生成带标注框的图。用法:make annotate URL=... OUT=out.png
	@if [ -z "$(URL)" ] && [ -z "$(FILE)" ]; then echo "✗ 请提供图片 URL= 或 FILE=" >&2; exit 1; fi
	@OUT="$(or $(OUT),annotated.png)"; \
	if [ -n "$(FILE)" ]; then \
		echo "→ 上传文件:$(FILE)" >&2; \
		curl -s --max-time 120 -F "file=@$(FILE)" "http://127.0.0.1:$(PORT)/analyze?annotate=true"; \
	else \
		echo "→ 下载 URL:$(URL)" >&2; \
		curl -s --max-time 120 -F "url=$(URL)" "http://127.0.0.1:$(PORT)/analyze?annotate=true"; \
...
help: ## 显示本帮助
	...
	@echo
	@echo "快速上手:make up && make test URL=https://你的图.png"
