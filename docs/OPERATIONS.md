...
`/analyze` 的性能告警已内置：请求耗时超过按图片大小动态计算的阈值、或处理失败时，自动推文本消息到飞书群。配置方法：

1. 飞书群 → 设置 → 群机器人 → 添加自定义机器人（安全设置选「加签」或「自定义关键词」，若选加签则把密钥也填入 `.env`）。
2. `.env` 里配置后重启服务：

```ini
OCR_FEISHU_WEBHOOK_URL=https://open.feishu.cn/open-apis/bot/v2/hook/xxxx
OCR_FEISHU_SECRET=            # 加签密钥，没开加签留空
OCR_ALERT_SLOW_BASE_SECONDS=30
OCR_ALERT_SLOW_SECONDS_PER_MP=5
OCR_ALERT_COOLDOWN_SECONDS=300
OCR_ALERT_ON_ERROR=true
```

阈值随分辨率线性放宽（OCR 耗时和像素数近似成正比，大图天然慢）：`阈值 = 30s + 5s × 兆像素数`。例：1000×800 → 34s，4000×3000 → 90s。两项全设 0 = 只告警失败不告警慢请求。慢/失败两类告警各自独立冷却（默认 5 分钟一条），防止刷屏。

告警内容包含总耗时、阈值、图片尺寸、task_id、来源 URL 以及 pipeline 分段耗时（pre/ocr/vlm/dedupe，`predict` 单列——它是 CPU 推理瓶颈的定位线索）。不配置 webhook 则完全不启用，对请求处理零影响。

「服务进程挂了」（而非单个请求慢/失败）仍需外部探活：任务计划程序每 5 分钟 `curl /healthz`，失败时同样可推飞书。
