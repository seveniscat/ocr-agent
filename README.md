...
image (100–10000px)
  ...
  ├─ dedupe (PaddleOCR-style seam merge + containment)
  ├─ optional VLM fallback (opt-in)
...
# 3. Configure
cp .env.example .env
#   → set OCR_VLM_API_KEY + OCR_VLM_ENABLED=true only if you need /understand or VLM fallback.
...
| `OCR_REC_CONFIDENCE_FALLBACK` | `0.95` | Below this → VLM re-reads crop if fallback enabled |
| `OCR_VLM_ENABLED` | `false` | Master switch for cloud VLM endpoints |
| `OCR_VLM_OCR_FALLBACK_ENABLED` | `true` | Low-confidence OCR crop re-read on `/analyze` (dormant while `VLM_ENABLED=false`) |
